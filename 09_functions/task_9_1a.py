...
Возвращает список всех портов в режиме access с конфигурацией на основе шаблона
	...
	for intf, vlan in intf_vlan_mapping.items():
		...
		for command in access_mode_template:
			if not command.endswith("access vlan"):
				result.append(command)
			else:
				result.append(f"{command} {vlan}")
		if psecurity: result.extend(port_security_template)
	return result
check = generate_access_config(access_config, access_mode_template, True)
print(check)
