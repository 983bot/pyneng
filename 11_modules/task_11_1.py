...
def parse_cdp_neighbors(command_output):
    ...
    """
    exclude = ['show cdp neighbors', 'Capability Codes', 'Repeater', 'Device ID']
    fltr=[line for line in command_output.split("\n") if not any(ex in line for ex in exclude) and not line.strip() == ""]
    hostname = command_output.split(">")[0].strip()
    result={}
    for lines in fltr:
        key=tuple(f"{hostname},{lines.split()[1]}{lines.split()[2]}".split(","))
        value=tuple(f"{lines.split()[0]},{lines.split()[-2]}{lines.split()[-1]}".split(","))
        result[key]=value
    return result
