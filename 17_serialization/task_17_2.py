...
def parse_sh_version(texttoparse):
    regexp=('IOS Software, .+?, Version (?P<ios>\S+), |uptime is (?P<uptime>.*)|image file is "(?P<image>\S+)"')
    for line in texttoparse.split('\n'):
...
def write_inventory_to_csv(data_filenames, csv_filename):
    ...
    for filename in data_filenames:
        ...
        with open(filename) as f:
            for i in parse_sh_version(f.read()):
                prs.append(i)
        result.append(prs)
...
if __name__=="__main__":
    write_inventory_to_csv(sh_version_files, 'res2.txt')
