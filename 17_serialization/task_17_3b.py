...
def transform_topology(filename):
    ...
    with open(filename) as f:
        ...
        for router, environment in topology.items():
            for interface, neighbor in environment.items():
                for nghost, nginterface in neighbor.items():
                    if not (nghost, nginterface) in res:
                        res[(router, interface)] = (nghost, nginterface)
    return res
